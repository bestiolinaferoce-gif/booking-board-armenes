...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (webhookUrl) {
      const event = (current?.data?.length ?? 0) < bookings.length ? 'created' : bookings.length === 0 ? 'cancelled' : 'modified';
      fetch(webhookUrl, {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({ event, property: 'bnb-armenes', bookings, timestamp: new Date().toISOString() }),
      }).catch(() => {});
    }

...
export async function GET(req: NextRequest) {
  ...
  const events = filtered.map((b) => {
    ...
    const lines = [
      ...
      foldLine(`LOCATION:B&B Armenes — ${escapeIcal(b.lodge)}`),
      `STATUS:${status}`,
  ...
  const cal = [
    'BEGIN:VCALENDAR',
    'VERSION:2.0',
    'PRODID:-//B&B Armenes//Booking Board//IT',
    'CALSCALE:GREGORIAN',
    'METHOD:PUBLISH',
    foldLine('X-WR-CALNAME:B&B Armenes — Booking Board'),
    'X-WR-TIMEZONE:Europe/Rome',
    'X-WR-CALDESC:Prenotazioni B&B Armenes',
    ...events,
    'END:VCALENDAR',
  ].join('\r\n');
